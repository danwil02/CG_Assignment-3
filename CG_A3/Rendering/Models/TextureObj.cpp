...
void TextureObj::Create(std::string inputfile)
{
...
void TextureObj::Draw()
{
	...
	//glUniform1i(shadowHandle, 3); // turn shadow code off for this

	for (int i = m_vao.size() - 1; i >= 0; i--)
	{
		if (m_texId[i] != -1)
		{
...
void TextureObj::quickSort(int left, int right) {
	int i = left, j = right;
	float pivot;
...
void TextureObj::createLight(float objMaxCoord)
{
	...
	float lightPos[4] = { 544, 378, -120, 1.0f };
