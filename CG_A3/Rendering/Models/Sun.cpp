...
void Sun::Create()
{
	...
	string inputfile = "Rendering\\Models\\TestObjs\\Sun\\sun.obj";
...
void Sun::Update(unsigned int flags)
{
...
void Sun::Draw()
{
	...
	m_modleViewMat = m_scaleMat * m_posOffset * m_posMat;
...
void Sun::sortShapes()
{
	...
	{
		glm::vec4 v = m_cameraManager->getProjectionMat() * m_modleViewMat * glm::vec4(m_shapePos[i], 1);
		m_shapePos[i] = glm::vec3(v);
	}

	// Update distances from camera to shapes//
	glm::vec3 cam = m_cameraManager->getCameraXYZ();
...
void Sun::quickSort(int left, int right) {
	int i = left, j = right;
	float pivot;
...
void Sun::setTexUnit(unsigned int unit)
{
...
// Textures //
void Sun::setupTexture(GLuint& texId, tinyobj::material_t material, string basePath)
{
	string texPath;
	int x, y, n;
...
void Sun::setupLightingProperties(tinyobj::material_t material)
{
	// Lighting Variables //
	...
	if (mtlambientHandle == -1 ||
		...
		mtlspecularHandle == -1 ||
		shininessHandle == -1 ||
		emissionHandle == -1 ){ //alphaHandle == -1) {
...
void Sun::pushMaterialPropsToShader(int idx)
{
...
void Sun::createLight(float objMaxCoord)
{
	objMaxCoord = 1000;
	// Uniform lighting variables //
...
void Sun::pushLightingPropsToShader(int idx)
{
