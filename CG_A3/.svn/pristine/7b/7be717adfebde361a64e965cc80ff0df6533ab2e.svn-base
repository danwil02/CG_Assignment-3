...
*/
	...
	void InitGLUT::keyBoardReleaseCallBack(unsigned char key, int x, int y)
	{
		m_listener->notifyKeyboardReleaseEvent(key, x, y);
	}
	...
	void InitGLUT::specialKeyReleaseCallBack(int key, int x, int y)
	{
		m_listener->notifySpecialKeyReleaseEvent(key, x, y);
	}
